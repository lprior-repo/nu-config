...
# }
# --- Individual Completer Definitions ---

# Carapace Completer (primary completer)
...
# Fish Completer (for git, nu, and other commands where Fish excels)
let fish_completer = {|spans: list<string>|
    try {
        /home/linuxbrew/.linuxbrew/bin/fish --private -c $"complete --do-complete='($spans | str join " ")'"
        | str trim
        | lines
        | each {|line|
        ...
        # Fallback to carapace if fish fails
        $carapace_completer | do $in $spans
    }
}

# Inshellisense Completer (for advanced AI-powered completions)
let inshellisense_completer = {|spans: list<string>|
    try {
        let line = ($spans | str join " ")
        inshellisense completion $line
        | lines
        | each {|completion| { value: $completion } }
    } catch {
        # Fallback to carapace if inshellisense fails
        $carapace_completer | do $in $spans
    }
}

# Zoxide Completer (for directory navigation)
let zoxide_completer = {|spans|
    $spans | skip 1 | zoxide query -l ...$in | lines | where {|x| $x != $env.PWD}
}

# --- Main External Completer (The Router) ---
...
let external_completer = {|spans|
    ...
    # The router logic: choose the best completer for the job
    match $spans.0 {
        # Fish is great for Git, Nushell, and development tools
        'nu' | 'git' | 'asdf' => $fish_completer

        # Inshellisense for modern tools and complex commands
        'docker' | 'kubectl' | 'terraform' | 'ansible' => $inshellisense_completer

        # Zoxide for directory navigation
        '__zoxide_z' | '__zoxide_zi' => $zoxide_completer

        # Carapace is the default powerhouse for everything else
...
$env.config = {
    ...
    # filesize config options were removed in newer nushell versions
...
# =============================================================================
# Load AWS profile management system (Nushell 0.108+ compatible)
use ~/.config/nushell/aws-profile.nu *

# Legacy aliases for backward compatibility
alias aws-status = aws-whoami
alias aws-clear = aws-logout
alias aws-select = aws-select

# HD-specific login alias
export def --env hd-login [
    --sso           # Use SSO login for HD profile
    --duration: int = 3600  # Session duration
] {
    # Determine which profile to use (you can customize this)
    let hd_profile = "default"  # Change this to your actual HD profile name

    print $"🏢 HD Login - Authenticating with profile: (ansi cyan)($hd_profile)(ansi reset)"

    # Run the authentication
    if $sso {
        aws-login $hd_profile --sso --duration $duration
    } else {
        aws-login $hd_profile --duration $duration
    }

    # Show final status
    print "\n📋 HD Authentication Status:"
    aws-whoami
}
